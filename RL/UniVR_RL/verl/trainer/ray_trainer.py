...
class RayPPOTrainer:
    ...
    def _validate(self) -> dict[str, Any]:
        if self.val_dataloader is None:
            print("[Trainer] val_dataloader is None, skipping validation.")
            return {}
        reward_tensor_lst = []
        ...
        for batch_dict in self.val_dataloader:
            ...
            test_gen_batch = test_batch.pop(
                ...
                non_tensor_batch_keys=["raw_prompt_ids", "multi_modal_data", "uncond_prompt_ids", "gt_frames_info",
                                       "decoded_gt_images_bytes", "decoded_ref_images_bytes"],
            )
            ...
            # [Memory Fix] Remove decoded images after reward is computed - no longer needed
            for _img_key in ["decoded_images", "decoded_reference_images", "decoded_gt_images",
                             "decoded_gt_images_bytes", "decoded_ref_images_bytes",
                             "gt_frames_info", "dinov2_scores"]:
                test_batch.non_tensor_batch.pop(_img_key, None)
    ...
    def _make_batch_data(self, metrics: dict[str, Any]) -> DataProto:
        ...
        while True:
            ...
            meta_info = {
                ...
                # Surface the trainer's step number so the EmuAgentic rollout
                # can name its per-step trace / image directory accordingly.
                "global_step": int(getattr(self, "global_step", 0)),
                "experiment_name": str(getattr(self.config.trainer, "experiment_name", "exp")),
            }
            ...
            gen_batch = new_batch.pop(
                ...
                non_tensor_batch_keys=["raw_prompt_ids", "multi_modal_data", "uncond_prompt_ids", "gt_frames_info",
                                       "decoded_gt_images_bytes", "decoded_ref_images_bytes"],
                meta_info_keys=["min_pixels", "max_pixels", "video_fps", "global_step", "experiment_name"],
            )
    ...
    def fit(self):
        ...
        while self.global_step < self.training_steps:
            ...
            with timer("step", timing_raw):
                ...
                print(f"\n[Timing] Step {self.global_step} | Rollout (gen): {timing_raw['gen']:.2f}s")
                ...
                _balance_time = _time_module.time() - _t0
                timing_raw["balance_batch"] = _balance_time
                print(f"[Timing] Step {self.global_step} | Balance batch: {_balance_time:.2f}s")
                ...
                if "token_level_scores" not in batch.batch:
                    ...
                    print(f"[Timing] Step {self.global_step} | Reward dispatch: {timing_raw['reward']:.2f}s")

                # [Memory Fix] Remove decoded images from batch - they are only needed by reward_fn
                # which was already dispatched above. Keeping them in batch causes unnecessary
                # serialization through Ray for every subsequent worker call.
                for _img_key in ["decoded_images", "decoded_reference_images", "decoded_gt_images",
                                 "decoded_gt_images_bytes", "decoded_ref_images_bytes",
                                 "gt_frames_info", "dinov2_scores"]:
                    batch.non_tensor_batch.pop(_img_key, None)

                # recompute old_log_probs
                with timer("old", timing_raw):
                    old_log_probs = self.actor_rollout_ref_wg.compute_log_probs(batch)
                    batch = batch.union(old_log_probs)
                print(f"[Timing] Step {self.global_step} | Old log_probs: {timing_raw['old']:.2f}s")
                ...
                if self.use_reference_policy:
                    ...
                    print(f"[Timing] Step {self.global_step} | Ref log_probs: {timing_raw['ref']:.2f}s")
                ...
                if self.use_critic:
                    ...
                    print(f"[Timing] Step {self.global_step} | Critic values: {timing_raw['values']:.2f}s")

                with timer("adv", timing_raw):
                    if "token_level_scores" not in batch.batch:
                        # get token level scores asynchronously
                        _reward_wait_start = _time_module.time()
                        reward_tensor, reward_metrics = ray.get(reward_ref)
                        _reward_wait_time = _time_module.time() - _reward_wait_start
                        timing_raw["reward_wait"] = _reward_wait_time
                ...
                print(f"[Timing] Step {self.global_step} | Advantage (incl. reward_wait={timing_raw.get('reward_wait', 0):.2f}s): {timing_raw['adv']:.2f}s")
                ...
                if self.use_critic:
                    ...
                    print(f"[Timing] Step {self.global_step} | Update critic: {timing_raw['update_critic']:.2f}s")
                ...
                if self.config.trainer.critic_warmup <= self.global_step:
                    ...
                    print(f"[Timing] Step {self.global_step} | Update actor: {timing_raw['update_actor']:.2f}s")
                ...
                if (
                    ...
                    print(f"[Timing] Step {self.global_step} | Validation: {timing_raw['validation']:.2f}s")

                if self.config.trainer.save_freq > 0 and self.global_step % self.config.trainer.save_freq == 0:
                    with timer("save_checkpoint", timing_raw):
                        self._save_checkpoint()
                    print(f"[Timing] Step {self.global_step} | Save checkpoint: {timing_raw['save_checkpoint']:.2f}s")

            # Print timing summary for this step
            _step_total = _time_module.time() - _step_start
            print(f"\n{'='*80}")
            print(f"[Timing Summary] Step {self.global_step} | Total: {_step_total:.2f}s")
            _phase_order = ["gen", "balance_batch", "reward", "old", "ref", "values", "adv", "reward_wait", "update_critic", "update_actor", "validation", "save_checkpoint"]
            _summary_parts = []
            for _pname in _phase_order:
                if _pname in timing_raw:
                    _summary_parts.append(f"{_pname}={timing_raw[_pname]:.2f}s")
            # Include any extra keys not in _phase_order
            for _pname, _pval in timing_raw.items():
                if _pname not in _phase_order and _pname != "step":
                    _summary_parts.append(f"{_pname}={_pval:.2f}s")
            print(f"[Timing Summary] Breakdown: {' | '.join(_summary_parts)}")
            print(f"{'='*80}\n")
            ...
            # [Memory Fix] Explicitly free batch data to avoid GPU memory accumulation.
            # Without this, the old batch lingers until Python GC runs, causing
            # CUDA caching allocator to hold fragmented memory blocks.
            del batch
            import gc
            gc.collect()
