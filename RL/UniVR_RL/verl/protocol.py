...
class DataProto:
    ...
    def chunk(self, chunks: int) -> list["DataProto"]:
        ...
        for key, value in self.non_tensor_batch.items():
            # Use Python list slicing to handle inhomogeneous data (e.g., variable-length sequences)
            chunk_size = len(value) // chunks
            non_tensor_lst = [value[i * chunk_size : (i + 1) * chunk_size] for i in range(chunks)]
    ...
    def concat(data: list["DataProto"]) -> "DataProto":
        ...
        for key, value in non_tensor_batch.items():
            # Use Python list concatenation to handle inhomogeneous data
            if isinstance(value[0], np.ndarray):
                try:
                    non_tensor_batch[key] = np.concatenate(value, axis=0)
                except ValueError:
                    # Fallback for inhomogeneous arrays (e.g., variable-length sequences)
                    non_tensor_batch[key] = [item for sublist in value for item in sublist]
            else:
                # For lists or other iterables, use list concatenation
                non_tensor_batch[key] = [item for sublist in value for item in sublist]
    ...
    def reorder(self, indices: torch.Tensor) -> None:
        ...
        for key, value in self.non_tensor_batch.items():
            if isinstance(value, np.ndarray):
                reordered_non_tensor_batch[key] = value[indices_np]
            else:
                # For Python lists, use list comprehension
                reordered_non_tensor_batch[key] = [value[i] for i in indices_np]
        self.non_tensor_batch = reordered_non_tensor_batch
    ...
    def repeat(self, repeat_times: int, interleave: bool = True) -> "DataProto":
        ...
        for key, value in self.non_tensor_batch.items():
            if isinstance(value, np.ndarray):
                ...
                # For Python lists
                if interleave:
                    # Repeat each element: [a, b] -> [a, a, b, b] for repeat_times=2
                    repeated_non_tensor_batch[key] = [item for item in value for _ in range(repeat_times)]
                else:
                    # Tile the list: [a, b] -> [a, b, a, b] for repeat_times=2
                    repeated_non_tensor_batch[key] = value * repeat_times
