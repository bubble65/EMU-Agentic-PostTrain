...
from .rollout import _lazy_emu_agentic, vLLMRollout
...
class FSDPWorker(Worker):
    def __init__(
        ...
        self.role = role
        self._cache = {}
        self._is_emu3 = False  # Will be set during model loading

        if not dist.is_initialized():
            dist.init_process_group(backend="nccl", timeout=timedelta(seconds=18000))
    ...
    def _check_and_register_emu3(self, model_path: str) -> bool:
        """
        Check if the model is an Emu3 model and register it to transformers AutoClass.
        
        Args:
            model_path: Path to the model
            
        Returns:
            True if this is an Emu3 model, False otherwise
        """
        import os
        import json
        ...
        if os.path.exists(config_file):
            try:
                ...
                is_emu3 = model_type in ("emu3", "emu3.5")
            except Exception:
                pass
        
        if is_emu3:
            self._is_emu3 = True
            self.print_rank0(f"[Emu3] Detected Emu3 model at {model_path}")
            
            # Add Emu_VW/src to path first
            import sys
            emu_src_path = "../UniVR_SFT/src"
            if os.path.exists(emu_src_path) and emu_src_path not in sys.path:
                sys.path.insert(0, emu_src_path)
                self.print_rank0(f"[Emu3] Added {emu_src_path} to Python path")
            
            # Register Emu3 to AutoClass with force=True to override built-in
            try:
                from ..models.emu3 import register_emu3_model
                register_emu3_model(emu_src_path=emu_src_path, force=True)
                self.print_rank0("[Emu3] Successfully registered Emu3 model to transformers AutoClass")
            except Exception as e:
                self.print_rank0(f"[Emu3] Warning: register_emu3_model failed: {e}, trying direct registration")
                # Fallback: directly register
                try:
                    from emu3p5 import Emu3ForCausalLM, Emu3Config
                    from transformers.models.auto.configuration_auto import CONFIG_MAPPING
                    from transformers.models.auto.modeling_auto import MODEL_FOR_CAUSAL_LM_MAPPING
                    
                    # Clear existing registrations from both _mapping and _extra_content
                    for key in ["emu3", "Emu3"]:
                        if hasattr(CONFIG_MAPPING, "_mapping") and key in CONFIG_MAPPING._mapping:
                            del CONFIG_MAPPING._mapping[key]
                        if hasattr(CONFIG_MAPPING, "_extra_content") and key in CONFIG_MAPPING._extra_content:
                            del CONFIG_MAPPING._extra_content[key]
                    
                    # Also clear from MODEL_FOR_CAUSAL_LM_MAPPING
                    ...
                    self.print_rank0("[Emu3] Registered Emu3Config and Emu3ForCausalLM")
                except Exception as e2:
                    raise RuntimeError(
                        f"[Emu3] Failed to register Emu3 model to transformers AutoClass. "
                        f"Primary error: {e}. Fallback error: {e2}. "
                        f"Please ensure 'loguru' is installed: pip install loguru"
                    ) from e2
        else:
            self._is_emu3 = False
    ...
    def _build_model_optimizer(
        ...
        if role != "ref":  # ref model's tokenizer is same as actor
            ...
            # Check if this is an Emu3 model and register it
            is_emu3 = self._check_and_register_emu3(model_config.model_path)
            ...
            is_emu3 = getattr(self, '_is_emu3', False)

        if padding_free:
            # Emu3 may not have ulysses patch support yet, skip if Emu3
            model_type = getattr(self.model_config, 'model_type', '')
            if model_type.lower() not in ('emu3', 'emu3.5'):
                apply_ulysses_patch(self.model_config.model_type)
                self.print_rank0("Ulysses patch applied!")
            else:
                self.print_rank0("[Emu3] Skipping Ulysses patch for Emu3 model")
        ...
        elif 'emu3' in getattr(self.model_config, 'model_type', '').lower():
            # Emu3 model - use direct import
            from emu3p5 import Emu3ForCausalLM
            AutoClass = Emu3ForCausalLM
        else:
            AutoClass = AutoModelForCausalLM
        print("model_config.model_path:", model_config.model_path)
        if (not fsdp_config.enable_rank0_init) or self.device_mesh.get_local_rank("fsdp") == 0:
            model = AutoClass.from_pretrained(
                ...
                device_map="auto" if self._is_emu3 else ("cpu" if fsdp_config.enable_rank0_init else "cuda"),
            ...
            with no_init_weights(), init_empty_weights():
                if self._is_emu3:
                    # Emu3: use direct instantiation with config (no weight loading for non-rank0)
                    from emu3p5 import Emu3ForCausalLM
                    model = Emu3ForCausalLM(self.model_config)
                else:
    ...
    def _build_rollout(self) -> None:
        rollout_name = getattr(self.config.rollout, "name", "vllm")
        if rollout_name not in {"vllm", "emu_agentic"}:
            raise ValueError(f"Unsupported rollout backend: {rollout_name!r}")
        ...
        lora_kwargs = (
            {"lora_kwargs": {"enable_lora": True, "max_loras": 1, "max_lora_rank": self._lora_rank, "lora_extra_vocab_size": 0}}
            if self._is_lora
            else {}
        )
        rollout_cls = _lazy_emu_agentic() if rollout_name == "emu_agentic" else vLLMRollout
        self.rollout = rollout_cls(
            model_path=self.config.actor.model.model_path,
            config=self.config.rollout,
            tokenizer=self.tokenizer,
            processor=self.processor,
            tokenizer_path=self.config.actor.model.tokenizer_path,  # Pass tokenizer path for Emu3
            **lora_kwargs,
        )
    ...
    def update_actor(self, data: DataProto):
        ...
        # [Memory Fix] Reset peak memory stats so we track per-step peak, not historical peak
        torch.cuda.reset_peak_memory_stats()
        # NOTE: Do NOT call torch.cuda.empty_cache() here!
        # PyTorch's caching allocator reuses memory blocks from previous operations.
        # Calling empty_cache() forces new cudaMalloc calls which can fragment GPU memory,
        # causing CUBLAS_STATUS_ALLOC_FAILED even when total free memory is sufficient.
        # See: verl/workers/sharding_manager/fsdp_vllm.py comment about empty_cache scope.
        ...
        # NOTE: offload_fsdp_model/offload_fsdp_optimizer above already call empty_cache().
        # The next vLLM wake_up in load_vllm_and_sync_weights also calls empty_cache().
        # No need for an extra empty_cache() here - it would be redundant.
        return output
    ...
    def compute_log_probs(self, data: DataProto):
        ...
        output = output.to("cpu")
        # NOTE: offload_fsdp_model above already calls empty_cache() internally.
        # Avoid extra empty_cache() to preserve PyTorch's caching allocator block layout.
        return output
