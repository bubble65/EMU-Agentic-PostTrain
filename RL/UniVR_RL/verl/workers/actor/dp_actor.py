...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _forward_micro_batch(self, micro_batch: dict[str, torch.Tensor], temperature: float) -> torch.Tensor:
        ...
        if self.config.padding_free:
            ...
            # [Memory Optimization] Use num_logits_to_keep to only compute logits for response tokens.
            # For Emu3.5 (vocab_size=282926, seq_len=14096), this avoids materializing the full
            # [bs, 14096, 282926] logits tensor, saving ~2.3GB per micro-batch in bf16.
            # Models that don't support num_logits_to_keep will silently ignore it via **kwargs.
            try:
                output = self.actor_module(
                    input_ids=input_ids,
                ...
                logits: torch.Tensor = output.logits  # (bsz, response_length + 1, vocab_size)
                logits.div_(temperature)
                logits = logits[:, :-1, :]  # (bsz, response_length, vocab_size)
            except TypeError:
                # Fallback for models that don't accept num_logits_to_keep
                output = self.actor_module(
                    input_ids=input_ids,
                    attention_mask=attention_mask,
                    position_ids=position_ids,
