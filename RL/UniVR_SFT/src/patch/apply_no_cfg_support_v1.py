...
def patch_logits_processor(builtin_path: Path):
    ...
    new_apply = '''    def apply(self, logits):
        # NO_CFG_SUPPORT_PATCHED: Support both CFG (paired requests) and no-CFG (single request) modes
        # CFG_PAIRING_FIX: Use _cfg_pairs for robust pairing instead of assuming index order

        if not self.metadata:
            return logits

        # CFG_PAIRING_FIX: Use _cfg_pairs cache for robust pairing in CFG mode
        # This ensures correct conditional/unconditional matching even when batch order is shuffled
        if self._cfg_pairs:
            processed_pairs = set()
            ...
            for pair_id, pair_dict in self._cfg_pairs.items():
                ...
                # Get conditional and unconditional indices for this pair
                cond_idx = pair_dict.get(True)  # is_conditional=True
                uncond_idx = pair_dict.get(False)  # is_conditional=False
                
                # Skip if pair is incomplete (one request finished before the other)
                if cond_idx is None or uncond_idx is None:
                    continue
                
                # Validate indices are within logits bounds
                ...
                format_token_ids = cond_meta.format_token_ids
                in_visual = cond_meta.in_visual
                in_image = cond_meta.in_image
                guidance_scale = self.guidance_scale.get(cond_idx, 1.0)
                
                if len(format_token_ids) > 0:
                    # Format token forcing - apply to conditional only
                    mask = torch.ones_like(logits[cond_idx], dtype=torch.bool)
                    mask[format_token_ids] = False
                    logits[cond_idx].masked_fill_(mask, float("-inf"))
                elif in_image and in_visual:
                    # CFG guidance for visual tokens
                    cond_logits = torch.nn.functional.log_softmax(logits[cond_idx], dim=-1)
                    uncond_logits = torch.nn.functional.log_softmax(logits[uncond_idx], dim=-1)
                    guided_logits = uncond_logits + guidance_scale * (cond_logits - uncond_logits)
                    ...
                    # Resolution prediction - top1 sampling
                    top1_idx = torch.argmax(logits[cond_idx], dim=-1, keepdim=True)
                    mask = torch.ones_like(logits[cond_idx], dtype=torch.bool)
                    mask.scatter_(dim=-1, index=top1_idx, value=False)
                    logits[cond_idx].masked_fill_(mask, float("-inf"))
                elif not in_image and not in_visual:
                    # Text generation - mask visual tokens
                    mask = torch.ones_like(logits[cond_idx], dtype=torch.bool)
                    mask[:self.visual_token_start_index] = False
                    logits[cond_idx].masked_fill_(mask, float("-inf"))
            ...
            # No-CFG mode: process each request individually without CFG guidance
            # Note: metadata keys are the logits indices themselves (0, 1, 2, ...)
            for i, metadata in self.metadata.items():
                if i >= logits.shape[0]:
                    continue
                if metadata is None:
                    continue
                ...
                if len(format_token_ids) > 0:
                    # Apply format token masking
                    mask = torch.ones_like(logits[i], dtype=torch.bool)
                    mask[format_token_ids] = False
                    logits[i].masked_fill_(mask, float("-inf"))
                elif in_image and in_visual:
                    # In visual mode without CFG - apply log_softmax to match CFG=1.0 behavior
                    logits[i] = torch.nn.functional.log_softmax(logits[i], dim=-1)
                    mask = torch.ones_like(logits[i], dtype=torch.bool)
                    mask[self.visual_token_start_index:] = False
                    logits[i].masked_fill_(mask, float("-inf"))
                elif in_image and not in_visual:
                    # Resolution mode - use top1 sampling
                    top1_idx = torch.argmax(logits[i], dim=-1, keepdim=True)
                    mask = torch.ones_like(logits[i], dtype=torch.bool)
                    mask.scatter_(dim=-1, index=top1_idx, value=False)
                    logits[i].masked_fill_(mask, float("-inf"))
                elif not in_image and not in_visual:
                    # Text mode - mask visual tokens
                    mask = torch.ones_like(logits[i], dtype=torch.bool)
                    mask[:self.visual_token_start_index] = False
                    logits[i].masked_fill_(mask, float("-inf"))
