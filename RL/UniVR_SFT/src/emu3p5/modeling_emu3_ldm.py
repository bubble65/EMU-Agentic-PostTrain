...
from .discrete_video_latent_dynamic import CausalDiscreteVideoLatentDynamicTokenizer
...
class Emu3ForCausalLMWithLDM(Emu3PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        # ---- LDM (Latent Dynamic Model) ----
        ldm_config = getattr(config, "ldm_config", None)
        if ldm_config is not None:
            self.ldm = CausalDiscreteVideoLatentDynamicTokenizer(**ldm_config)
            # Project LDM latent codes into the LLM hidden dimension.
            # The connector inside LDM outputs 1024-d; map to hidden_size.
            ldm_output_dim = ldm_config.get("connector_output_dim", 1024)
            self.ldm_proj = nn.Linear(ldm_output_dim, config.hidden_size)
            self.ldm_loss_weight = getattr(config, "ldm_loss_weight", 1.0)
        else:
            self.ldm = None
            self.ldm_proj = None
            self.ldm_loss_weight = 0.0

        # Initialize weights and apply final processing
        self.post_init()

    # ------------------------------------------------------------------
    # Helpers for freezing / unfreezing LDM sub-modules
    # ------------------------------------------------------------------
    def freeze_ldm_encoder_decoder(self):
        """Freeze the LDM encoder & decoder so only the connector / quantizer trains."""
        if self.ldm is None:
            return
        for name, param in self.ldm.named_parameters():
            if "quant_to_dit_dim" not in name:
    ...
    def forward(
        ...
        pixel_values_aug: Optional[torch.FloatTensor] = None,
        ldm_positions: Optional[torch.LongTensor] = None,
        ...
        # ------------------------------------------------------------------
        # LDM encoding: produce latent dynamics embeddings from raw images
        # ------------------------------------------------------------------
        ldm_loss = None
        ldm_embeds = None  # (batch, num_ldm_tokens, hidden_size)
        if self.ldm is not None and pixel_values is not None:
            ldm_output = self.ldm(pixel_values, input_aug=pixel_values_aug)
            if isinstance(ldm_output, dict):
                quant_codes = ldm_output["quant_codes"]
                ldm_loss = ldm_output["quant_loss"]
            else:
                quant_codes = ldm_output.quant_codes
                ldm_loss = ldm_output.quant_loss

            # Map LDM latent codes → LLM hidden dim via the connector
            # quant_codes shape: (B, C, T, 1, 1) → flatten → (B, T, C)
            if self.ldm.connector_type == "llama":
                # LlamaConnector already outputs (B, seq_len, out_channels)
                ldm_embeds = self.ldm.quant_to_dit_dim(quant_codes)
            else:
                # ConvBlock connector outputs (B, C, T, H, W) → flatten
                connector_out = self.ldm.quant_to_dit_dim(quant_codes)
                ldm_embeds = connector_out.flatten(2).permute(0, 2, 1)

            # Project to LLM hidden size
            ldm_embeds = self.ldm_proj(ldm_embeds)  # (B, num_ldm_tokens, hidden_size)

        # ------------------------------------------------------------------
        # Build inputs_embeds: merge LDM embeddings into the token sequence
        # ------------------------------------------------------------------
        if ldm_embeds is not None and input_ids is not None and ldm_positions is not None:
            # ldm_positions: (B, num_ldm_tokens) — indices in the token sequence
            #                where LDM embeddings should be placed.
            if inputs_embeds is None:
                inputs_embeds = self.model.embed_tokens(input_ids)
            for b in range(inputs_embeds.shape[0]):
                num_ldm_tok = min(ldm_embeds.shape[1], ldm_positions.shape[1])
                pos = ldm_positions[b, :num_ldm_tok]
                inputs_embeds[b, pos] = ldm_embeds[b, :num_ldm_tok]
            # We already built inputs_embeds; do not pass input_ids again.
            input_ids = None
        ...
        # Combine LDM quantization loss with the language modeling loss
        if ldm_loss is not None:
            if isinstance(ldm_loss, dict):
                # Some quantizers return a dict of losses; sum them.
                total_ldm_loss = sum(v for v in ldm_loss.values() if isinstance(v, torch.Tensor))
            elif isinstance(ldm_loss, torch.Tensor):
                total_ldm_loss = ldm_loss
            else:
                total_ldm_loss = torch.tensor(0.0, device=logits.device)

            if loss is not None:
                loss = loss + self.ldm_loss_weight * total_ldm_loss
            else:
                loss = self.ldm_loss_weight * total_ldm_loss
