...
class Emu3ForCausalLM(Emu3PreTrainedModel):
    ...
    def forward(
        ...
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
        # print("labels:", labels.tolist(), "input_ids:", input_ids.tolist())
        ...
        # For RL training, we only need logits for response tokens, not the full prompt.
        # This saves ~(prompt_len * vocab_size * dtype_size) bytes of GPU memory.
        # E.g., with prompt_len=4096, vocab_size=282926, bf16: saves ~2.3GB per micro-batch.
        ...
        logits = logits.float()
        # [Memory Fix] Removed logits = logits.float() which was converting
        # [bs, seq_len, 282926] from bf16 to fp32, doubling memory from ~8GB to ~16GB.
        # The downstream loss computation (flash-attn cross_entropy or F.cross_entropy)
        # handles the float conversion internally when needed.
