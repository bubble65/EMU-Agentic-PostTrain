...
from .qformer_ldm import QFormerMF, QFormerMFSep, QFormerMFSingleQ, PositionEmbeddingRandom, QFormerMFAdjacent, QFormerAdjacentFSingleQ
...
class CausalCrossAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, context: torch.Tensor) -> torch.Tensor:
        ...
        h_w = bhw // batch_size
        k = k.unsqueeze(1).expand(-1, h_w, -1, -1).reshape(bhw, context_T, c)
        v = v.unsqueeze(1).expand(-1, h_w, -1, -1).reshape(bhw, context_T, c)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i_level in range(self.num_resolutions):
            ...
            if i_level != self.num_resolutions - 1:
                ...
                hs.append(
                    F.avg_pool3d(hs[-1], kernel_size=[time_factor, 1, 1], stride=[2, 1, 1])
                )
        ...
        **ignore_kwargs,
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            for _ in range(self.num_res_blocks + 1):
                block.append(
                    CausalResnetBlock3d(
                        in_channels=block_in, out_channels=block_out, dropout=dropout, num_groups=num_groups,
                    )
                )
                block_in = block_out
                if curr_res in attn_resolutions:
    ...
    def __init__(
        ...
        **ignore_kwargs,
        ...
        for i_level in range(self.num_resolutions):
            ...
            for _ in range(self.num_res_blocks):
                block.append(
                    CausalResnetBlockFactorized3d(in_channels=block_in, out_channels=block_out, dropout=dropout, num_groups=1)
                )
                block_in = block_out
                if curr_res in attn_resolutions:
                    attn.append(nn.Sequential(
...
class LDMEncoderFactorized(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        dense_pe = torch.repeat_interleave(dense_pe, bz, dim=0)
        query = torch.repeat_interleave(self.act.weight[None], bz, dim=0)

        if h.shape[2] > 1:
            query = self.qformer_ldm(h, dense_pe, query)
            query = self.outproj_ldm(query.permute(0, 2, 1).unsqueeze(-1).unsqueeze(-1))
        else:
            query = None
    ...
    def __init__(
        ...
        **ignore_kwargs,
        ...
        for _ in range(2):
            ...
            downsample = CausalHybridDownsample3d(block_in, spatial_down=True, temporal_down=False)
