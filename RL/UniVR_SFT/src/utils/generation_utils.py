...
def multimodal_decode(
    ...
    image_count = 0  # Track image index in the sequence
    for c in chunks:
        if len(c) == 0:
            continue
        if tokenizer.boi_token in c and tokenizer.eoi_token in c:
            image = decode_image(c, tokenizer, vision_tokenizer, image_index=image_count)
            image_count += 1
            if image is not None:
                multimodal_output.append(("image", image))
            else:
                print(f"Error adding Generated image #{image_count}")
...
def decode_image(image_string, tokenizer, vision_tokenizer, image_index=None):
    image: List[List[int]] = []
    ...
    if len(image) > 0:
        ...
        expected_width = row_lengths[0]  # Use first row as reference
        inconsistent_rows = []
        for row_idx, length in enumerate(row_lengths):
            if length != expected_width:
                inconsistent_rows.append((row_idx, length))
        
        if inconsistent_rows:
            img_id_str = f" (Image #{image_index})" if image_index is not None else ""
            print(f"[WARNING] Inconsistent visual token counts detected!{img_id_str}")
            print(f"  Expected width (from row 0): {expected_width}")
            print(f"  Total rows: {len(image)}")
            for row_idx, length in inconsistent_rows:
                diff = length - expected_width
                print(f"  Row {row_idx}: {length} tokens ({'+' if diff > 0 else ''}{diff})")
