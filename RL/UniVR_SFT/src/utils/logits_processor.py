...
class UnbatchedClassifierFreeGuidanceLogitsForVisualTokenProcessor(LogitsProcessor):
    ...
    def __init__(
        ...
        # Check if CFG is disabled (guidance_scale <= 1.0 means no CFG effect)
        self.cfg_disabled = (guidance_scale <= 1.0)
        if self.cfg_disabled:
            print(f"[INFO] CFG disabled (guidance_scale={guidance_scale}), skipping unconditional computation for 2x speedup")
    ...
    def __call__(self, input_ids, scores):
        ...
        if input_ids[0][-1] == BOI:
            ...
            self.in_image = True  # Always set in_image flag
        # TEXT MODE
    ...
    def in_image_logits_processor(self, input_ids, scores):
        # If CFG is disabled, skip unconditional computation entirely
        if self.cfg_disabled:
            return self._in_image_logits_processor_no_cfg(input_ids, scores)
    ...
    def _in_image_logits_processor_no_cfg(self, input_ids, scores):
        """Fast path when CFG is disabled - skip all unconditional computation."""
        if self.in_visual:
            # generating visual tokens
            img_idx = self.find_last_token_index(input_ids[0], IMG)
            vis_idx = input_ids.shape[1] - img_idx
            ...
            # eol
                ...
                scores = scores + mask
                return scores
            # visual token - NO CFG applied, just mask non-visual tokens
            else:
                mask = torch.full_like(scores, -math.inf)
                mask[:, BOV:] = 0
                scores = scores + mask
            ...
            if input_ids[0][-1] == IMG:
                self.in_visual = True
                self.image_nums += 1
                print(f"[INFO] Generating image#{self.image_nums} (no CFG)...")
                if self.first_in_image or not self.force_same_image_size:
                    if self.height is None or self.width is None:
                        self.parse_hw(input_ids)
                    self.first_in_image = False
                
                # the first visual token - NO CFG applied
                mask = torch.full_like(scores, -math.inf)
                mask[:, BOV:] = 0
                scores = scores + mask
                return scores
    ...
    def set_unconditional_context(self, input_ids):
        # Note: in_image flag is now set in __call__ method
    ...
    def exit_image(self, input_ids):
        self.in_image = False
        self.in_visual = False
        # update eoi to unconditional cache (skip if CFG is disabled)
        if not self.cfg_disabled:
