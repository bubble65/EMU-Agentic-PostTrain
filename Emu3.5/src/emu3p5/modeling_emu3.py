...
class Emu3ForCausalLM(Emu3PreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None and self.training:
            ...
            total_loss = hidden_states.new_tensor(0.0, dtype=torch.float32)
            total_tokens = shift_labels.ne(-100).sum().clamp_min(1)

            chunk_size = getattr(self.config, "loss_chunk_size", 1024)
            for start in range(0, hidden_states.size(1) - 1, chunk_size):
                end = min(start + chunk_size, hidden_states.size(1) - 1)
                chunk_hidden = hidden_states[:, start:end, :]
                chunk_logits = self.lm_head(chunk_hidden).float()
                chunk_labels = shift_labels[:, start:end].to(chunk_logits.device)

                total_loss = total_loss + loss_fct(
